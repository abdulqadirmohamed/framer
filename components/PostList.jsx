...
const PostList = ({ title, cover, tag, id }) => {
  return (
    <Link href={`blog/${id}`} className="mt-10 cursor-pointer group">
      <div className="relative overflow-hidden">
        <div className="absolute left-0 top-[-50px] m-3  flex gap-2 group-hover:top-0 z-50 ">
          <span className="bg-lightBlue px-2 rounded-sm mx-2 ">{tag}</span>
        </div>
        <Image
          src={cover}
          alt="image"
